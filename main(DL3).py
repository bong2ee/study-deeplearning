...
print("-------------------------------------------------")
# 각 정보별 특징 자세히 출력하기 (샘플수, 평균, 표준편차, 최솟값, 백분위 수로 25%,50%,75%에 해당하는 값,최댓값)
print(df.describe())
...
# 그래프 속성
'''
    시본 라이브러리 중 각 항목 간 상관관계를 나타내는 heatmap() 함수 통해 그래프 표시
...
# plasma를 기준으로 각각 정상과 당뇨가 어느 정도 비율 분포 확인하기
'''
...
# bml를 기준으로 각각 정상과 당뇨가 어느 정도 비율로 분포 확인하기
'''
    BMI가 높아질 경우 당뇨의 발병률도 함께 증가하는 추세임
    평균이나 중앙값으로 대치하거나, 이상치를 제거 과정 등이 데이터 전처리에 포함가능 (성능높임)
'''
