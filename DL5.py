'''
    1. 퍼셉트론(perceptron)
        ...
        3) 딥러닝은 은닉충이 2개 이상인 인공신경망 모형의 학습하는 것임
        4) 은닉층과 출력층
            - 각 입력값을 처리하는 합성함수와 출력으로 보내기 위한 활성화함수로 구성

    2. 역전파(Backpropagation)
        1) 가중치와 편향을 학습하기 위한 방법으로 고안(1974, 1986)
            - 역전파가 XOR 문제뿐만 아니라 더 복잡한 문제도 해결할 수 있음이 증명됨 ==> 인공신경망 다시 관심 시작
        2) 신경망의 오차(예측값과 실제값의 차이)를 출력층에서 입력층으로 거꾸로
           전파시켜 각 층(layer)의 가중치와 편향을 업데이트함
        3) 신경망이 깊어질수록 학습력이 좋아져야 하는데 기대하는 결과가 나오지 않는 일이 지속적 발생

    3. 딥(Deep) 출현
        1) 2006년, 가중치의 초기값을 제대로 설정하면 깊이가 다른
...
def perceptron_fit(X, Y, epochs=10):
    ...
    for t in range(epochs):
        ...
        for i in range(len(X)):
            ...
            print('현재 처리 입력값 : ', X[i], ', 정답 : ', Y[i], ', 출력(뉴런 활성화 여부) : ', predict, ', 변경 된 가중치 : ', W)
        print("---------------------------------------------------------------")

perceptron_fit(X, y, 2)