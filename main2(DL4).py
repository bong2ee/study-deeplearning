...
# astype() 메서드를 이용하여 데이트를 범주형(단어)으로 변환
...
print("-" * 30)
total_records = 1728
test_records = int(total_records * .2)  # 데이터셋을 훈련셋과 테스트 셋 용도로 분리
...
'''
    ...
    이 DL4 경우 데이터를 분류해야 하는 것으로 크로스 엔트로피(cross entropy) 손실함수를 사용함
    옵티마이저는 아담(Adam)을 사용함.
    
    lr(Learning Rate) : 미분값을 얼마만큼 이동시킬 것인가를 결정함.
                        초기값이 크다면 초반에 loss값이 빠르게 줄것임. 나중에 가면 underfitting이 발생함 
'''
print("-" * 30)
...
# 신경망에서 필요한 모든 파라미터를 무작위로 선택했다는 감안할때 74%의 정확도는 나쁘지 않음
# 파라미터(훈련/테스트 데이터 셋 분할, 은닉층 개수 및 크기 등) 변경하면서 더 나은 성능을 찾아보는 것도 의미가 있을것임
